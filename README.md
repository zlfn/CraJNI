...
# 빌드
이걸 할 사람이 있겠냐만은 src/main/rust를 cargo build로 빌드하여 얻은 rust.so(dll)을 적당히 어디 넣고 gradle build로 빌드된 CraJNI.jar을 plugins 폴더에 넣은 Bukkit계열 서버에서 rust.so(dll)
)을 -Djava로 링크하여 실행하면 됩니다.
